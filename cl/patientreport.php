...
<div class="card-block">
  <div class="row">
      <div class="col-lg-12">
        ...
        <?php 
          if($_SESSION['user']=='patient'){
            $sqlappointment1 = "SELECT max(appointmentid) FROM appointment where patientid='$_SESSION[patientid]' AND (status='Active' OR status='Approved')";
    ...
    <?php
    if($_SESSION['user']=='patient'){
      $sql ="SELECT * FROM report WHERE patientid='$_SESSION[patientid]'";
    }
    else{
      $sql ="SELECT * FROM report WHERE patientid='$_GET[patientid]'";
    }
      $qsql = mysqli_query($conn,$sql);
